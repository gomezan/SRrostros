...
#obtener unicamente componente Y
...
#Conversion a componentes YCbCr
...
#Normaliza y obtiene componente y
...
def posprocess(y,cb,cr):
    ...
    #unir los canales de color
    output = np.array([pos,cb,cr]).transpose([1, 2, 0])
...
def posprocessRGB(r,g,b):
    ...
    posB = b.mul(255.0).cpu().numpy().squeeze(0).squeeze(0)
                    
    #unir los canales de color
    frame= np.array([posR, posG, posB]).transpose([1, 2, 0]).astype(np.uint8)
    return frame
